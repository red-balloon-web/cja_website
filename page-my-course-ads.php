...
get_header(); ?>
	...
	<div id="primary" class="content-area">
		...
		if ($_POST['update-ad']) {
			...
			$cja_update_ad->save(); 
			?><p class="cja_alert cja_alert--success">Your advert for "<?php echo ($cja_update_ad->title); ?>" has been updated.<span class="right"><a href="<?php echo get_site_url() . '/' . $cja_config['my-course-ads-slug']; ?>"> Return to My Adverts</a></span></p><?php
		}
		 ...
		 *  - Extend ad
		 */
		include('inc/my-course-adverts/get-functions.php');
								...
								if ($currentad->status == 'active') {
									if ($cja_current_user->credits > 0) { 
										if (get_option('cja_charge_users') || $currentad->days_left < 35) { ?>
											<div class="cja_icon cja_extend_button cja_call_are_you_sure" data-text="Extend Advert" data-yaybutton="Extend" data-linkurl="<?php echo ($cja_page_address . '?extend-course-ad=' . get_the_ID()); ?>"><i class="fas fa-clock cja_tooltip"><div class="tooltiptext">extend</div></i></div><?php 
										}
									}
