...
get_header(); ?>
	...
	<div id="primary" class="content-area">
		<main id="main" class="site-main" role="main"><?php
			...
			/**
			 * JOBSEEKER SECTION
			 * If user is an applicant display the section relevant to them
			 */
			if ($cja_current_user_obj->role == 'jobseeker') {
				...
				if ($_GET['applicant_archive']) {
					...
					?><p class="cja_alert cja_alert--success">You archived your application to '<?php echo $cja_archive_application_advert->title; ?>' at <?php echo $cja_archive_application_advert->author_human_name; ?>.</p><?php
				} ?>

				<h1>Job Applications I Have Made</h1>
				
				<!-- filter to show archived or current applications -->
				<div class="application-filter-options">
					<form action="<?php echo get_site_url() ?>/applications-jobs" method="get">
						<p>Show: 
			 ...
			 */
				...
				// If we're not looking at a specific job then display all our jobs
				
				if (!$_GET['advert_id']) { ?>

					<h1>Applications to My Jobs</h1> <?php
					...
					if ($query->have_posts()) {
						while ($query->have_posts()) {
							...
							// For each job display the number of applications and a link to go to the page of applications for just that job
							$query->the_post();
							$cja_advert = new CJA_Advert(get_the_id());

							// Display items in table ?>
							<div class="cja_list_item">
								<a href="<?php echo get_site_url(); ?>/applications-jobs?advert_id=<?php echo $cja_advert->id; ?>" class="cja_icon"><i class="fas fa-eye cja_tooltip"><div class="tooltiptext">view applications</div></i></a>
					...
					$cja_advert = new CJA_Advert($_GET['advert_id']); ?>
					<h1>Applications for <?php echo $cja_advert->title; ?></h1>
					<p><a href="<?php echo get_site_url(); ?>/applications-jobs"><< Back to my job applications</a></p>
					...
					<p><a href="<?php echo get_site_url(); ?>/wp-admin/admin-post.php?action=create-job-applicants-csv&advert_id=<?php echo $cja_advert->id; ?>" class="cja_button">Export Applicants as CSV</a></p>
					<p>&nbsp;</p>
					...
					if ($query->have_posts()) {
						while ($query->have_posts()) { 
							$query->the_post();
							$application = new CJA_Application(get_the_ID());
							?>
							<div class="cja_list_item">
