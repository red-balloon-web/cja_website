...
get_header(); ?>
	...
	<div id="primary" class="content-area">
		...
		// update ad if form info is posted back
		if ($_POST['update-ad']) {
		...
		// Display the advert list if we are displaying it
		if ($do_list) {
			...
			<!--<div class="my-jobs-header">
				<div class="cja_credits_remaining"><span class="credits-large"><?php 
					...
					</span>&nbsp;&nbsp;classified advert credits remaining
				</div><?php 
				if ($cja_current_user->classified_credits > 0) { ?>
			...
			if ($cja_current_user->classified_credits < 1) { ?>
								...
								if ($currentad->status == 'active') {
									if ($cja_current_user->credits > 0) { 
										if (get_option('cja_charge_users') || $currentad->days_left < 35) { ?>
											<div class="cja_icon cja_extend_button cja_call_are_you_sure" data-text="Extend Advert" data-yaybutton="Extend" data-linkurl="<?php echo ($cja_page_address . '?extend-classified-ad=' . get_the_ID()); ?>"><i class="fas fa-clock cja_tooltip"><div class="tooltiptext">extend</div></i></div><?php 
										}
									}
								// else display reactivate icon
								} else if ($currentad->status == 'expired' && $cja_current_user->credits > 0) { ?>
									<div class="cja_icon cja_extend_button cja_call_are_you_sure" data-text="Reactivate Advert" data-yaybutton="Activate" data-linkurl="<?php echo ($cja_page_address . '?activate-ad=' . get_the_ID()); ?>"><i class="fas fa-clock cja_tooltip"><div class="tooltiptext">reactivate</div></i></div><?php
								} ?>
							</div>
...
jQuery(document).ready(function() {
	...
	// Fade out the modal if user clicks outside the box
	...
	// Fade out the modal if user clicks on no button
